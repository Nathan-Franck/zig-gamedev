        ...
        .Vector => |vectorInfo| {
            switch (token) {
                .ArrayBegin => {
                    ...
                    errdefer {
                        // Without the r.len check `r[i]` is not allowed
                        if (vectorInfo.len > 0) while (true) : (i -= 1) {
...
pub fn parseFree(comptime T: type, value: T, options: ParseOptions) void {
    switch (@typeInfo(T)) {
        ...
        .Vector => |vectorInfo| {
            inline for (0..vectorInfo.len) |i| {
                parseFree(vectorInfo.child, value[i], options);
            }
        },
        .Void => {},
        else => unreachable,
...
pub fn stringify(
    ...
    switch (@typeInfo(T)) {
        ...
        .Void => return stringify(null, options, out_stream),
        else => @compileError("Unable to stringify type '" ++ @typeName(T) ++ "'"),
    }
    unreachable;
}
